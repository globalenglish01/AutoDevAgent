...
### 3.2 代码操作工具（原计划 `code-mcp` MCP 服务器 → 实际用 Python 原生工具）

TestAgent 现有 3 个 MCP 服务器（api/ui/perf）都是"调用被测系统"的工具，没有一个是"操作代码仓库本身"的工具。AutoDevAgent 需要新建一套。计划清单：
...
**2026-07-08 Phase 2 实现时的架构调整（重要，偏离了"Node.js MCP 服务器"的原计划）**：

原计划把这些做成一个独立的 Node.js MCP 服务器（`code-mcp`，结构参照 TestAgent 的 `backend/mcp/api`）。实现时改成 **Python 进程内原生 LangChain 工具**，原因：

1. **`fs.*` 不用自己写**——deepagents 传入 `backend` 后，`FilesystemMiddleware` 自动带出 `read_file`/`write_file`/`edit_file`/`ls`/`glob`/`grep`，且 `FilesystemBackend(virtual_mode=True)` 已经拦截路径穿越（Phase 1 已验证）。重复造轮子没有意义。
2. **`git.*` 和 StaticCheck 本质是 Python/CLI 工具**——git 是命令行，ruff/mypy/py_compile 是 Python 模块。如果套一个 Node.js 进程再 shell 出去调 Python，等于多一个语言运行时 + 多一次 IPC 往返，纯亏。
3. **MCP 独立进程模式留给真正需要隔离的场景**——即第 5 节的 `shell.exec`（Phase 5），那里进程/容器隔离才有意义。

Phase 2 实际交付（均在 `orchestrator/tools/`，21 个测试全绿，工具测试真实跑 ruff/mypy/git 非 mock）：

| 文件 | 内容 |
|---|---|
| `path_guard.py` | 写路径守卫（第 5.4 节）：拒绝穿越 workspace 根目录、拒绝 `.env`/`*.pem`/`id_rsa`/`*.key` 等敏感文件名。被 git/staticcheck 及以后的写工具共用 |
| `staticcheck.py` | **StaticCheck 三工具**（Python only）：`syntax_check`(compileall)/`lint_check`(ruff)/`typecheck_run`(mypy)，绑定到 workspace，返回 PASS/FAIL + 报错，供 CodeAgent 自检 |
| `git_tools.py` | `git_status`/`git_diff`/`git_create_branch`（拒 main/master）/`git_commit`（暂存到敏感文件时拒绝提交并撤回暂存）。`git_push` 有意不在 Phase 2 暴露（隔离测试仓库无远程，且 push 的凭据/分支守卫归 Phase 3+ 的 DeployAgent） |
| `agents/code/agent.py` | **CodeAgent 骨架**：把上述工具 + deepagents 内置 fs 工具接入 `create_deep_agent`，绑定到真实目标仓库（不是 hello_world 那种一次性空目录）。system prompt 写明工作纪律：先切 feature 分支→小步改→每步 syntax→lint→typecheck 自检→全绿再 commit |

未做（留待后续阶段）：`shell.exec`（Phase 5 沙箱）、`pkg.install`、JS/TS 静态检查、`git_push`。
